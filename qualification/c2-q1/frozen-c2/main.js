...
const { app, BrowserWindow, ipcMain, nativeTheme, globalShortcut, screen, clipboard, shell, dialog, protocol, Tray, Menu, powerMonitor } = require('electron');
const { GenericApprovalGuard } = require('./modules/trusted-client/genericApprovalGuard');
const c2GenericApprovalGuard = new GenericApprovalGuard();
let c2TrustedIntegration = null;
...
if (!gotTheLock) {
    ...
    app.whenReady().then(async () => { // Make the function async
        ...
        const c2Trusted = c2TrustedIntegration ||= require('./modules/trusted-client/bootstrap').install({ app, BrowserWindow, ipcMain, powerMonitor, dialog, shell, projectRoot: __dirname, readSettings: () => appSettingsManager.readSettings() });
        const menuTemplate = [
            c2Trusted.menu,
            ...(isMac ? [{
                label: app.name,
    ...
    function connectVcpLog(wsUrl, wsKey) {
        ...
        vcpLogWebSocket.onmessage = (event) => {
            ...
            try {
                ...
                c2GenericApprovalGuard.observe(data);
    ...
    ipcMain.on('send-vcplog-message', (event, data) => {
        if (!c2GenericApprovalGuard.canSend(data)) return;
