...
最终，plugin-manifest.json 的注册名被更新为 TopicSponsor——语义更准确（"Sponsor"=发起者/赞助者，比"Creator"更能表达"主动发起对话"的含义）。当前执行入口已同步为 `topicsponsor.js`，日志前缀与元数据字段仍保留历史兼容层。
...
- **插件类型**：synchronous（同步）
- **通信协议**：stdio（标准输入/输出）
- **入口点**：`node topicsponsor.js`（manifest entryPoint 与正式插件名保持一致）
- **超时时间**：15000ms（15秒）
...
| 脚本文件名 | `topicsponsor.js` | 当前正式入口，manifest entryPoint 引用它 |
...
**为什么仍保留部分历史字段？** 因为：
1. 已创建话题的 history.json 中存储了旧字段名
2. 其他用户的提示词中可能引用了旧名字
3. 元数据字段全量迁移会增加运行时数据风险

**原则**：对外接口与执行入口使用新名字，历史元数据保留兼容层。
...
- **修复内容**：
  ...
  - topicsponsor.js checkTopicOwnership(): 读取端加 `topicSponsor || topicCreator` fallback
  - topicsponsor.js 日志前缀x2: `[AgentTopicCreator]` -> `[TopicSponsor/AgentTopicCreator]`
- **兼容保留内容**（保留历史地层）：
