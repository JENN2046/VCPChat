...
function renderVCPLogNotification(logData, originalRawMessage = null, notificationsListUl, themeColors = {}) {
    if (logData && typeof logData === 'object' && logData.type === 'tool_approval_request' && logData.data && typeof logData.data === 'object') {
        if (!window.vcpSensitiveAuthorityClassifier || window.vcpSensitiveAuthorityClassifier(logData.data)) {
            const item = document.createElement('li');
            item.textContent = 'This request requires Trusted Human Authorization. Use the Trusted Approval window.';
            notificationsListUl?.appendChild(item);
            return;
        }
