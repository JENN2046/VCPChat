...
#elif defined(__APPLE__)
SecKeyRef old=openKey();if(old){CFRelease(old);return fail(e,"IDENTITY_EXISTS");}CFDataRef t=tag();CFErrorRef error=nullptr;SecAccessControlRef ac=SecAccessControlCreateWithFlags(nullptr,kSecAttrAccessibleWhenUnlockedThisDeviceOnly,(diagnostic?0:kSecAccessControlPrivateKeyUsage),&error);if(!ac){CFRelease(t);if(error)CFRelease(error);return fail(e,"KEY_POLICY_FAILED");}const void* pk[]={kSecAttrIsPermanent,kSecAttrApplicationTag,kSecAttrAccessControl};const void* pv[]={kCFBooleanTrue,t,ac};CFDictionaryRef priv=CFDictionaryCreate(nullptr,pk,pv,3,&kCFTypeDictionaryKeyCallBacks,&kCFTypeDictionaryValueCallBacks);int bits=256;CFNumberRef size=CFNumberCreate(nullptr,kCFNumberIntType,&bits);const void* ks[]={kSecAttrKeyType,kSecAttrKeySizeInBits,kSecAttrTokenID,kSecPrivateKeyAttrs};const void* vs[]={kSecAttrKeyTypeECSECPrimeRandom,size,kSecAttrTokenIDSecureEnclave,priv};CFDictionaryRef original=CFDictionaryCreate(nullptr,ks,vs,4,&kCFTypeDictionaryKeyCallBacks,&kCFTypeDictionaryValueCallBacks);CFMutableDictionaryRef attrs=CFDictionaryCreateMutableCopy(nullptr,0,original);CFRelease(original);if(diagnostic){CFDictionaryRemoveValue(attrs,kSecAttrTokenID);CFMutableDictionaryRef software=CFDictionaryCreateMutableCopy(nullptr,0,priv);CFDictionaryRemoveValue(software,kSecAttrAccessControl);CFDictionarySetValue(attrs,kSecPrivateKeyAttrs,software);CFRelease(software);}SecKeyRef key=SecKeyCreateRandomKey(attrs,&error);CFRelease(attrs);CFRelease(size);CFRelease(priv);CFRelease(ac);CFRelease(t);long platformStatus=error?CFErrorGetCode(error):0;if(error)CFRelease(error);if(!key)return failStatus(e,"KEY_CREATE_FAILED",platformStatus);CFRelease(key);return identity(e,i);
#else
