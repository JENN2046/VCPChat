...
- 不直接把 PR12 合并到 `custom` 或后续单独定义的 VCPChat 生产线。
...
- `git diff --check`
- 本地等价 CI：已存在的关键 JS 文件 `node --check`
- `npm run test:photo-studio --if-present`
- 说明：PR-A 是前置文档/CI 批次，Photo Studio 代码尚未进入时，workflow 会跳过缺失文件和缺失脚本；后续 PR-B / PR-C 合入相关文件后，检查会自动覆盖到这些文件。
...
- 不建议把 VCPChat 推广动作指向 `A:\VCP\VCPToolBox-prod-stable`；该路径属于 VCPToolBox 后端项目稳定生产线。
- 不建议把 Photo Studio、legacy 迁移、AI image 实验和二进制更新混在同一个正式 PR 内。

## 当前阻塞

- `npm run pack` 被本机 `winCodeSign` 符号链接权限和依赖卫生阻塞。
- VCPChat 生产线尚未单独预检；此前提到的 `prod/stable` / `Plugin/UserAuth/code.bin` 属于 VCPToolBox 后端项目口径，不是 VCPChat 当前目标线。
