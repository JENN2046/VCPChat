...
(async()=>{
 ...
 for(const what of ['native','main','trusted-main','preload','html','ui-js','ui-css']){
  const copy=path.resolve('tamper-'+what);fs.cpSync(appdir,copy,{recursive:true});const p=path.join(copy,resources,what==='native'?'app.asar.unpacked/native/build/Release/qualification_provider.node':'app.asar');if(what!=='native'){const a=require('@electron/asar'),tmp=path.resolve('tamper-extracted');a.extractAll(p,tmp);fs.appendFileSync(path.join(tmp,({main:'runtime.cjs','trusted-main':'frozen-c2/modules/trusted-client/trustedDecisionBoundary.js',preload:'frozen-c2/preloads/trusted-approval.js',html:'frozen-c2/modules/trusted-client/ui/approval.html','ui-js':'frozen-c2/modules/trusted-client/ui/approval.js','ui-css':'frozen-c2/modules/trusted-client/ui/approval.css'})[what]),'\n// qualification tamper\n');await a.createPackageWithOptions(tmp,p,{unpack:'native/**/*.node'});fs.rmSync(tmp,{recursive:true,force:true});}else{const fd=fs.openSync(p,'r+'),b=Buffer.alloc(1);fs.readSync(fd,b,0,1,0);b[0]^=1;fs.writeSync(fd,b,0,1,0);fs.closeSync(fd);}const r=execute(copy,'TARGET','tamper-'+what);records.push({case:'tamper-'+what,...r});assert.notEqual(r.status,0);assert.notEqual(r.report?.result,'PASS_BOUNDED_PACKAGED_COMPONENT_TESTS');if(what==='native')assert.match(r.report?.error||'',/NATIVE_ARTIFACT_MISMATCH/);else assert.match(fs.readFileSync(path.resolve('evidence/tamper-'+what+'/process.log'),'utf8'),/integrity|hash|code sign|signature/i,'Must observe integrity rejection, not incidental startup failure');fs.rmSync(copy,{recursive:true,force:true});
 }
